            ...
            <tbody>
                @forelse ($allSlotDetails as $slot)
                    <tr>
                        ...
                        <td>
                            <div class="actions">
                                <a class="btn btn-sm bg-success-light" data-id ="{{ $slot['name'] }}"
                                    data-bs-toggle="modal" href="#edit_slot"
                                    onclick="edit_slot('{{ $slot }}', '{{ $slot->doctorExceptionDays->pluck('exception_days_id') }}')">
